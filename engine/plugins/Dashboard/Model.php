...
class Model
{
    ...
    public function createOrUpdateDashboard($login, $idDashboard, $layout)
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            // Note key is both login and iddashboard
            $query = 'BEGIN
		        BEGIN
			        UPDATE ' . $this->table . ' SET layout = ? WHERE login = ? and iddashboard = ?
		        END
		        IF NOT EXISTS (SELECT * FROM ' . $this->table . ' WHERE login = ? and iddashboard = ?)
			        BEGIN
			            INSERT INTO  ' . $this->table . ' (login, iddashboard, layout) VALUES (?,?,?)
			        END
	          END';
            $bind = array($layout, $login, $idDashboard, $login, $idDashboard, $login, $idDashboard, $layout);
        } else {
