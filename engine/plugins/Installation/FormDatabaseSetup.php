...
class Rule_checkUserPrivileges extends HTML_QuickForm2_Rule
{
        ...
        switch ($adapter) {
            case 'PDO\MSSQL':
                return array(
                    ...
                    'CREATE TEMPORARY TABLES' => 'CREATE TABLE #' . self::TEST_TEMP_TABLE_NAME . ' (
                                                id INT IDENTITY(1,1),
                                                PRIMARY KEY (id)
                                             )',
                );
    ...
    {
        $db->query('DROP TABLE IF EXISTS ' . self::TEST_TABLE_NAME);
        $db->query('DROP TABLE IF EXISTS ' . self::TEST_TEMP_TABLE_NAME);
    }
}
