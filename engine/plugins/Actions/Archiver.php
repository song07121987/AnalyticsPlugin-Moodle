...
class Archiver extends \Piwik\Plugin\Archiver
{
    ...
    protected function archiveDayActions($rankingQueryLimit)
    {
        // if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            // do not impose limits as query becomes very complex
        //    $rankingQueryLimit = 0;
        // }
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $select = "
                    ROW_NUMBER() over (ORDER BY count(*) DESC, log_action.name ASC) as CNTR,
                    log_action.name,
                    log_action.type,
                    log_action.idaction,
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $groupBy = "log_action.idaction,log_action.name, log_action.type, log_action.url_prefix";
            // taken care by row num
            $orderBy = '';
            // $orderBy = "`" . PiwikMetrics::INDEX_PAGE_NB_HITS . "` DESC, name ASC";
        } else {
            // mysql has weird grouping by
    ...
    protected function archiveDayEntryActions($rankingQueryLimit)
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $select = "
				ROW_NUMBER() over (ORDER BY count(*) DESC, log_action.name ASC) as CNTR,
                log_visit.%s as idaction, $extraSelects
    ...
    protected function archiveDayExitActions($rankingQueryLimit)
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $select = "
				ROW_NUMBER() over (ORDER BY count(*) DESC) as CNTR,
                log_visit.%s as idaction, $extraSelects
    ...
    protected function archiveDayActionsTime($rankingQueryLimit)
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $select = "
                ROW_NUMBER() over (ORDER BY count(*) DESC) as CNTR,
                log_link_visit_action.%s as idaction, $extraSelects
        ...
        $groupBy = "log_link_visit_action.%s, idaction, log_action.type, log_action.name";
