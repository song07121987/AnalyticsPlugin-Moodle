...
class Archiver extends \Piwik\Plugin\Archiver
{
    ...
    protected function aggregateDayEvents()
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $select = "
		    ROW_NUMBER() over (ORDER BY count(distinct log_link_visit_action.idvisitor) DESC) as CNTR,
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $groupBy = "
		    log_action_event_name.name,
		    log_action_event_category.name ,
		    log_action_event_action.name,
		    log_action_event_category.idaction,
