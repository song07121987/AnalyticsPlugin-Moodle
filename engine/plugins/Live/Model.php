...
class Model
{
    ...
    public function queryActionsForVisit($idVisit, $actionsLimit)
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $sql .= ' OFFSET 0 ROWS FETCH NEXT '.$actionsLimit.' ROWS ONLY';
        } else {
            $sql .= " LIMIT 0, $actionsLimit";
        }
    ...
    public function queryEcommerceItemsForOrder($idVisit, $idOrder, $actionsLimit)
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $sql .= ' ORDER BY idvisit ASC OFFSET 0 ROWS FETCH NEXT '.$actionsLimit.' ROWS ONLY';
        } else {
            $sql .= " LIMIT 0, $actionsLimit";
        }
	...
	// Stage 1
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $sql = "SELECT TOP 1 sub.idvisitor, sub.visit_last_action_time FROM ({$queryInfo['sql']}) as sub
    ...
    public function makeLogVisitsQueryString($idSite, $period, $date, $segment, $offset, $limit, $visitorId, $minTimestamp, $filterSortOrder)
    {
        ...
        // Issue with group by as mssql does not support partial group by
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $sql = "
			SELECT sub.* FROM (
				" . $subQuery['sql'] . "
