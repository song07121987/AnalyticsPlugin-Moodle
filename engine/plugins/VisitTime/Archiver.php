...
class Archiver extends \Piwik\Plugin\Archiver
{
    ...
    protected function aggregateByServerTime()
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $dataArray = $this->getLogAggregator()->getMetricsFromVisitByDimension(
                array("label" => 'LEFT(CONVERT(TIME(0), log_visit.visit_last_action_time), 2)')
            );
            $query = $this->getLogAggregator()->queryConversionsByDimension(
                array("label" => 'LEFT(CONVERT(TIME(0), log_conversion.server_time), 2)')
            );
        } else {
    ...
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $array = $this->getLogAggregator()->getMetricsFromVisitByDimension("LEFT(CONVERT(TIME(0), log_visit.visitor_localtime), 2)");
        } else {
