...
class Archiver extends \Piwik\Plugin\Archiver
{
    ...
    private function aggregateDayImpressions()
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $select = "
		        ROW_NUMBER() over (ORDER BY count(distinct log_link_visit_action.idvisit) DESC) as CNTR,
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $groupBy = "
                    log_action_content_piece.name ,
                    log_action_content_target.name ,
                    log_action_content_name.name ,
                    log_action_content_piece.idaction,
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            // order by taken care of ranking
            $orderBy = "";
        } else {
    ...
    private function aggregateDayInteractions()
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $groupBy = "
                    log_action_content_name.name ,
                    log_action_content_interaction.name ,
                    log_action_content_piece.name ,
                    log_action_content_piece.idaction,
