...
class Model
{
    ...
    {
        if (CacheHelper::hasKey('site_all'))
            return CacheHelper::getValue('site_all');
        $sites = $this->getDb()->fetchAll("SELECT * FROM " . $this->table . " ORDER BY idsite ASC");
        CacheHelper::setValue('site_all', $sites);
        return $sites;
    }
    ...
    public function deleteSite($idSite)
    {
        ...
        CacheHelper::deleteValue('site_'.$idSite);
        $this->clearGlobalCache();
    }

    private function clearGlobalCache () {
        CacheHelper::deleteValue('sites_idall');
        CacheHelper::deleteValue('sites_all');
    }
    ...
    public function getSitesFromIds($idSites, $limit = false)
    {
        ...
        if ($limit) {
            if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                $limit = ' OFFSET 0 ROWS FETCH NEXT '.(int)$limit.' ROWS ONLY';
            } else {
    ...
    {
        if (CacheHelper::hasKey('site_'.$idSite))
            return CacheHelper::getValue('site_'.$idSite);
        $site = $this->getDb()->fetchRow("SELECT * FROM " . $this->table . "
                                          WHERE idsite = ?", $idSite);
        CacheHelper::setValue('site_'.$idSite, $site);
        return $site;
    }
    ...
    {
        if (CacheHelper::hasKey('sites_idall'))
            return CacheHelper::getValue('sites_idall');
        ...
        CacheHelper::setValue('sites_idall', $idSites);
        return $idSites;
    }
