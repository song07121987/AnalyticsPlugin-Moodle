...
class RankingQuery
{
    ...
    public function generateRankingQuery($innerQuery)
    {
        ...
        // this is moved up so we can do more groupings
        $groupBy = 'counter';
        ...
        foreach ($this->additionalColumns as $additionalColumn => $aggregation) {
            ...
            if ($aggregation !== false) {
                ...
                if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                    $groupBy .= ', `' . $additionalColumn . '`';
                }
                $additionalColumnsAggregatedString .= ', `' . $additionalColumn . '`';
            }
        }
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            // $groupBy .= $additionalColumnsString;
        }

        // Ignore this for mssql as we are already partiotining
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
        } else {
        ...
        // group by the counter - this groups "Others" because the counter stops at $limit
        // this has already been moved up
        // $groupBy = 'counter';

        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            // We have already added this column - no need to add again
        } else {
            if ($this->partitionColumn !== false) {
    ...
    private function getCounterExpression($limit)
    {
        ...
        if ($this->partitionColumn !== false) {
            ...
            foreach ($this->partitionColumnValues as $value) {
                ...
                if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                    $whens[] = "WHEN $isValue AND CNTR = $limit THEN $limit";
                    $whens[] = "WHEN $isValue THEN CNTR";
                } else {
            ...
            if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                ...
                $whens[] = "WHEN CNTR = $limit THEN $limit";
                $whens[] = "ELSE CNTR";
            } else  {
                // no partitioning: add a single counter
