        ...
        if (empty($deviceDetectionBlobAvailableDate)) {
            ...
            foreach ($archiveBlobTables as $table) {
                ...
                if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                    $deviceDetectionBlobAvailableDate = Db::get()->fetchOne(sprintf("SELECT TOP 1 date1 FROM %s WHERE name = 'DevicesDetection_browserVersions' AND period = 1 ORDER BY date1 ASC ", $table));
                } else {
