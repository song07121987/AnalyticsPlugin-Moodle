...
use Piwik\Session\SaveHandler\MongoTable;
use Zend_Session;
...
class Session extends Zend_Session
{
    ...
    public static function start($options = false)
    {
        ...
        elseif ($config->General['session_save_handler'] === 'mongodb'
            || in_array($currentSaveHandler, array('user', 'mm'))
        ) {
            $config = array(
                'collection'           => Common::prefixTable('session'),
            );
            $saveHandler = new MongoTable($config);
            if ($saveHandler) {
                self::setSaveHandler($saveHandler);
            }
        }
        elseif ($config->General['session_save_handler'] === 'wincache'
            || in_array($currentSaveHandler, array('user', 'mm'))
        ) {
            $sessionPath = self::getSessionsDirectory();
            Filesystem::mkdir($sessionPath);
