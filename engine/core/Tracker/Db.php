...
use Piwik\Tracker\Db\Pdo\Mssql;
...
abstract class Db
{
    ...
    public function recordProfiling()
    {
        ...
        foreach ($this->queriesProfiling as $query => $info) {
            ...
            if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                ...
                BEGIN
                    BEGIN
                        UPDATE ' . Common::prefixTable('log_profiling') . ' SET count = count + ' . $count . ', sum_time_ms = sum_time_ms + ' . $time . '  WHERE query = ?
                    END
                    IF NOT EXISTS (SELECT * FROM ' . Common::prefixTable('log_profiling') . ' WHERE query = ?)
                    BEGIN
                        INSERT INTO  ' . Common::prefixTable('log_profiling') . ' (query, count, sum_time_ms) VALUES (?, ' . $count . ', ' . $time . ')
			        END
		        END';
                $this->query($queryProfiling , array($query, $query, $query));
            } else {
     ...
     * @throws DbException if an exception occurred
     */
    abstract public function query($query, $parameters = array(), $ignore = false);
    ...
    public static function factory($configDb)
    {
        ...
        switch ($configDb['adapter']) {
            ...
            case 'PDO\MSSQL':
            case 'PDO_MSSQL':
                require_once PIWIK_INCLUDE_PATH . '/core/Tracker/Db/Pdo/Mssql.php';
                return new Mssql($configDb);
        }
