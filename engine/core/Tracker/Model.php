...
class Model
{
    ...
    private function insertNewAction($name, $type, $urlPrefix)
    {
        ...
        $db->query($sql, array($name, $crc32, $type, $urlPrefix));
    ...
    private function getSqlSelectActionId()
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $sql = "SELECT TOP 1 idaction, type, name FROM " . Common::prefixTable('log_action')
    ...
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $sql = sprintf('SELECT TOP 1 idsite FROM %s WHERE idsite = ? ', Common::prefixTable('log_visit'));
        } else {
