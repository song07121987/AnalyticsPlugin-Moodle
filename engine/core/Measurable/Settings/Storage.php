    ...
    public function save()
    {
        ...
        foreach ($this->settingsValues as $name => $value) {
            ...
            if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                ...
                BEGIN
                    BEGIN
                        UPDATE ' . $table . ' SET setting_value = ? WHERE idsite = ? and setting_name = ?
                    END
                    IF NOT EXISTS (SELECT * FROM ' . $table . ' WHERE idsite = ? and setting_name = ? )
                        BEGIN
                            INSERT INTO ' . $this->table . ' (idsite, setting_name, setting_value) VALUES (?, ?, ?)
                        END
                END';
                $bind = array($value, $this->idSite, $name, $this->idSite, $name, $this->idSite, $name, $value);
            } else {
