...
class Mssql extends Zend_Db_Adapter_Pdo_Mssql implements AdapterInterface
{
    ...
    public function getConnection()
    {
        ...
        try {
            ...
            */
            ...
            // print_r ($constr);
            if ($uid == "") {
                $this->_connection = new PDO("sqlsrv:server=$serverName;Database=$database");
            } else {
                $this->_connection = new PDO("sqlsrv:server=$serverName;Database=$database", $uid, $pwd);
            }
     ...
     */
    public function query($sql, $bind = array(), $ignore = false)
    {
    ...
    public function getDropTableSQL ($tables) {
        $sql = "";
        foreach ($tables as $tbl) {
            $sql .= "IF EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[".$tbl."]') AND TYPE IN (N'U')) DROP TABLE [dbo].[".$tbl."];";
        }
        return $sql;
    }

    public function getLastInsertId($tblname)
    {
