...
class LogQueryBuilder
{
    ...
    private function buildWrappedSelectQuery($select, $from, $where, $groupBy, $orderBy, $limit, $innerGroupBy = null)
    {
        ...
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            // We anyway should not group by for mssql
            // $innerGroupBy = false;
        }
    ...
    {
        $sql = "
			SELECT ".$this->appendTopLimitClauseToQuery ($limit)."
				$select
			FROM
				$from";
        ...
        $sql = $this->appendLimitClauseToQuery($sql, $limit, $orderexist);

        return $sql;
    }

    private function appendTopLimitClauseToQuery ($limit)
    {
        if ($limit == 0)
            return '';
    ...
    private function appendLimitClauseToQuery($sql, $limit, $orderexist = 1)
    {
        if ($limit == 0)
            return $sql;
        $limitParts = explode(',', (string) $limit);
        ...
        if ($isLimitWithOffset) {
            ...
            if ($limit >= 1) {
                if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
                    // Top should have covered this
                    // $sql .= sprintf(' OFFSET 0 ROWS FETCH NEXT  %d ROWS ONLY', $limit);
                } else {
                    $sql .= " LIMIT $limit";
                }
            }
