...
class LogAggregator
{
    ...
    public function queryEcommerceItems($dimension)
    {
        if (QueryHelper::DEFAULT_SCHEMA == 'Mssql') {
            $groupby =  "log_conversion_item.idorder, log_action.name, log_conversion_item.%s";
        } else {
            $groupby =  "ecommerceType, log_conversion_item.%s";
        }
        $query = $this->generateQuery(
            ...
            // GROUP BY ...
            // mssql version "log_conversion_item.idorder, log_action.name, log_conversion_item.%s",
            sprintf(
                $groupby,
                $dimension
            ),
