...
class Access
{
    ...
    {
        if (CacheHelper::hasKey('access_'.$login))
            return CacheHelper::getValue('access_'.$login);
        $sql = self::getSqlAccessSite("access, t2.idsite");
        $data = Db::fetchAll($sql, $login);
        CacheHelper::setValue('access_' . $login, $data);
        return $data;
    }
    ...
    public static function getSqlAccessSite($select)
    {
        ...
        $sql = 'SELECT ' . $select . ' FROM ' . $access . ' as t1 '
            . 'INNER JOIN ' . $siteTable . ' as t2 on t1.idsite = t2.idsite '
            . 'WHERE login = ?';
        return $sql;
